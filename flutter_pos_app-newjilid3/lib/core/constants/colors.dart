import 'package:flutter/material.dart';

/// Legacy color palette from the pre-redesign UI.
///
/// Do **not** add new references. New widgets must read from the
/// [AppPalette] ThemeExtension via `Theme.of(context).extension<AppPalette>()`
/// or `context.palette`. Migration mapping is in `.claude/rules/design-system.md` §9.
@Deprecated(
  'Use Theme.of(context).extension<AppPalette>() — see .claude/rules/design-system.md',
)
class AppColors {
  /// primary = #3949AB
  static const Color primary = Color(0xff3949AB);

  /// grey = #B7B7B7
  static const Color grey = Color(0xffB7B7B7);

  /// light = #F8F5FF
  static const Color light = Color(0xffF8F5FF);

  /// light = #C7D0EB
  static const Color blueLight = Color(0xffC7D0EB);

  /// black = #000000
  static const Color black = Color(0xff000000);

  /// white = #FFFFFF
  static const Color white = Color(0xffFFFFFF);

  /// green = #50C474
  static const Color green = Color(0xff50C474);

  /// red = #F4261A
  static const Color red = Color(0xffF4261A);

  /// card = #E5E5E5
  static const Color card = Color(0xffE5E5E5);

  /// disabled = #C8D1E1
  static const Color disabled = Color(0xffC8D1E1);
}
